...
proc userInput
    ...
    mainStaff := Pic.FileNew ("blank staff.jpg") %Get the staff picture
    var keyboard := Pic.FileNew ("keyboardshort.jpg") %Get the keyboard picture
    ...
    ledger := Pic.FileNew ("ledgerline.jpg")
    whole := Pic.FileNew ("wholenotesmaller.jpg") %Get notes pictures
    half := Pic.FileNew ("halfnoteupsmaller2.jpg") %23x56
    halfDown := Pic.Rotate (half, 180, 11, 28)
    quarter := Pic.FileNew ("quarternoteup.jpg") %50x116
    quarterDown := Pic.Rotate (quarter, 180, 25, 58)
    eighth := Pic.FileNew ("8thnoteupsingle.jpg") %77x116
    eighthDown := Pic.Rotate (eighth, 180, 38, 58)
    sixteenth := Pic.FileNew ("16thnoteupsingle.jpg") %47x87
	...
	if Mouse.ButtonMoved ("down") and my <= 145 then %If clicked the keyboard
